...
class LearningAgent(Agent):
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            if random.random() < self.epsilon:
                ...
                action=[k for k, v in self.Q[state].items() if v == maxQ][random.randint(0, len([k for k, v in self.Q[state].items() if v == maxQ])-1)]
        else:
            action = random.choice(self.valid_actions)
        return action
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning:
            ...
            new = reward
            self.Q[state][action]=self.alpha*new+(1-self.alpha)*old
    ...
    def update(self):
        ...
        if state[1]==False and action==None and ((state[2]<>'right' and state[0]=='forward') or (state[0]=='left' and state[2]==None)) and reward>0:
            reward = -reward
